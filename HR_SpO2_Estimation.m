...
%% START
clear all
[X1,X2]=textread('PPG.dat','%s %s'); %le todos os dados do arquivo ppg x1-940nm e x2-660nm

for i=1:length(X1) % faz um for com o npumero de vezes igual ao numero de dados
    X(i,1)=hex2dec(X1(i)); %converte de exadecimal para decimal 
    if X(i,1)> hex2dec('7FFFFF')   %Dont pay attention to this part. It is the conversion of acquired HEX value into useful decimal value
        X(i,1)=hex2dec('7FFFFF') - X(i,1);%se for mairo que 8388607 ele faz 8388607 menos o valor convertido
    end
    X(i,2)=hex2dec(X2(i)); 
...
plot(X(:,1))
hold on
plot(X(:,2),'r') %plota os gráficos

fs=25; %sampling rate 15Hz  %define como 15 amostras por segundo
FFT_size=128; %default: 64   %define o tamanho da
